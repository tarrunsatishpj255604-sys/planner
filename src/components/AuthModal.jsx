...
export default function AuthModal({ open, onClose, mode: initialMode }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setError(''); setEmail(''); setPassword(''); setResetSent(false)
    }
  }, [open, initialMode])
        ...
        {mode === 'reset' ? (
            ...
            {resetSent ? (
              <div className="auth-body">
                <div className="auth-success">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><path d="M22 4L12 14.01l-3-3" /></svg>
                  Check your inbox — we've sent a password reset link to your email.
                </div>
                <button className="btn btn-primary auth-submit" onClick={() => { setMode('signin'); setResetSent(false); setError('') }}>
                  Back to sign in
                </button>
              </div>
            ) : (
          ...
          <>
            ...
            <form className="auth-body" onSubmit={handleSubmit}>
              ...
              {mode === 'signin' && (
                <button type="button" className="auth-forgot-link" onClick={() => { setMode('reset'); setError(''); setPassword('') }}>
